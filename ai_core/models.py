...
class TTS(BaseModel):
    ...
    rate: int = Field(..., ge=-100, le=100, description="Rate in percentage (-100 to 100)")
...
class Visual(BaseModel):
    ...
    query: str = Field(..., description="Query for stock media or prompt for generated image")
    fallback_image_prompt: str = Field(..., description="Prompt for generating a fallback image if stock media is not found")
    shot: str = Field(..., description="Shot type (e.g., close-up, wide shot, etc.)")
    mood: str = Field(..., description="Mood of the visual (e.g., happy, dramatic, etc.)")


class Caption(BaseModel):
    text: str = Field(..., description="Caption text")
    position: str = Field(..., description="Position of the caption on the screen (e.g., top, bottom, center)")
    style: str = Field(..., description="Style of the caption (e.g., font size, color, etc.)")
...
class Scenario(BaseModel):
    ...
    style: str = Field(..., description="Style of the scenario (e.g., cinematic, documentary, etc.)")
