...
class DocumentStore:
    def __init__(
        ...
        self.vector_store = Chroma(
            ...
            persist_directory=os.getenv("CHROMA_DB_LOCATION") or "./chroma_db",
        )
    ...
    def add_document(
        self, campaign_id: int, document_id: UUID, filename: str, text: str
    ) -> None:
        ...
        chunk_ids = [
            f"{campaign_id}:{document_id}:{index}" for index in range(len(chunks))
        ]
    ...
    def search(self, campaign_id: int, query: str, limit: int = 5):
        return self.vector_store.similarity_search(
            query=query, k=limit, filter={"campaign_id": str(campaign_id)}
        )
    ...
    def update_document(
        self, campaign_id: int, document_id: UUID, new_text: str
    ) -> None:
